...
ExtMvc.OrderDetailListField = Ext.extend(Ext.form.Field, {
	initComponent: function () {
		...
		_gridPanel = new ExtMvc.OrderDetailGridPanel(_cfg);
		...
		Ext.apply(_this, {
			onRender: function (ct, position) {
				...
				ExtMvc.OrderDetailListField.superclass.onRender.call(_this, ct, position);
			...
			onResize: function (w, h, aw, ah) {
				ExtMvc.OrderDetailListField.superclass.onResize.apply(_this, arguments);
				_gridPanel.setSize(w, h);
			},
			onEnable: function () {
				ExtMvc.OrderDetailListField.superclass.onEnable.apply(_this, arguments);
				_gridPanel.enable();
			},
			onDisable: function () {
				ExtMvc.OrderDetailListField.superclass.onDisable.apply(_this, arguments);
				_gridPanel.disable();
			},
			beforeDestroy: function () {
				Ext.destroy(_gridPanel);
				ExtMvc.OrderDetailListField.superclass.beforeDestroy.apply(_this, arguments);
			},
			setValue: function (v) {
				_gridPanel.getStore().proxy.data.items = v;
				_gridPanel.getStore().load();
				return ExtMvc.OrderDetailListField.superclass.setValue.apply(_this, arguments);
			},
			getValue: function () {
		...
		ExtMvc.OrderDetailListField.superclass.initComponent.apply(_this, arguments);
	}
});
