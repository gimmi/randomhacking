...
ExtMvc.Ns.CustomerNormalSearchPanel = Ext.extend(Ext.Panel, {
	initComponent: function () {
		...
		_searchFormPanel = new ExtMvc.Ns.CustomerNormalSearchFormPanel({
		...
		_store = new Ext.data.Store({
			...
			proxy: new Rpc.JsonPostHttpProxy({
				url: 'NsCustomer/SearchNormal'
			}),
			remoteSort: true,
			reader: new ExtMvc.Ns.CustomerJsonReader(),
			listeners: {
		...
		_gridPanel = new ExtMvc.Ns.CustomerGridPanel({
			region: 'center',
			store: _store,
			bbar: _pagingToolbar,
			listeners: {
		...
		ExtMvc.Ns.CustomerNormalSearchPanel.superclass.initComponent.apply(_this, arguments);
