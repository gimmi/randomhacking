...
ExtMvc.Ns.EmployeeNormalSearchPanel = Ext.extend(Ext.Panel, {
	initComponent: function () {
		...
		_searchFormPanel = new ExtMvc.Ns.EmployeeNormalSearchFormPanel({
		...
		_store = new Ext.data.Store({
			...
			proxy: new Rpc.JsonPostHttpProxy({
				url: 'NsEmployee/SearchNormal'
			}),
			remoteSort: true,
			reader: new ExtMvc.Ns.EmployeeJsonReader(),
			listeners: {
		...
		_gridPanel = new ExtMvc.Ns.EmployeeGridPanel({
			region: 'center',
			store: _store,
			bbar: _pagingToolbar,
			listeners: {
		...
		ExtMvc.Ns.EmployeeNormalSearchPanel.superclass.initComponent.apply(_this, arguments);
