...
ExtMvc.CategoryListField = Ext.extend(Ext.form.Field, {
	initComponent: function () {
		...
		_gridPanel = new ExtMvc.CategoryGridPanel(_cfg);
		...
		Ext.apply(_this, {
			onRender: function (ct, position) {
				...
				ExtMvc.CategoryListField.superclass.onRender.call(_this, ct, position);
			...
			onResize: function (w, h, aw, ah) {
				ExtMvc.CategoryListField.superclass.onResize.apply(_this, arguments);
				_gridPanel.setSize(w, h);
			},
			onEnable: function () {
				ExtMvc.CategoryListField.superclass.onEnable.apply(_this, arguments);
				_gridPanel.enable();
			},
			onDisable: function () {
				ExtMvc.CategoryListField.superclass.onDisable.apply(_this, arguments);
				_gridPanel.disable();
			},
			beforeDestroy: function () {
				Ext.destroy(_gridPanel);
				ExtMvc.CategoryListField.superclass.beforeDestroy.apply(_this, arguments);
			},
			setValue: function (v) {
				_gridPanel.getStore().proxy.data.items = v;
				_gridPanel.getStore().load();
				return ExtMvc.CategoryListField.superclass.setValue.apply(_this, arguments);
			},
			getValue: function () {
		...
		ExtMvc.CategoryListField.superclass.initComponent.apply(_this, arguments);
	}
});

Ext.reg('ExtMvc.CategoryListField', ExtMvc.CategoryListField);