...
ExtMvc.Ns.CustomerDemographicNormalSearchPanel = Ext.extend(Ext.Panel, {
	initComponent: function () {
		...
		_searchFormPanel = new ExtMvc.Ns.CustomerDemographicNormalSearchFormPanel({
		...
		_store = new Ext.data.Store({
			...
			proxy: new Rpc.JsonPostHttpProxy({
				url: 'NsCustomerDemographic/SearchNormal'
			}),
			remoteSort: true,
		...
		_gridPanel = new ExtMvc.Ns.CustomerDemographicGridPanel({
			region: 'center',
			store: _store,
			bbar: _pagingToolbar,
			listeners: {
		...
		_onDeleteButtonClick = function () {
			...
			Ext.MessageBox.confirm('Delete', 'Are you sure?', function (buttonId) {
				...
				Rpc.call({
					url: 'NsCustomerDemographic/Delete',
		...
		ExtMvc.Ns.CustomerDemographicNormalSearchPanel.superclass.initComponent.apply(_this, arguments);
