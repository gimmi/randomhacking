...
ExtMvc.EmployeeListField = Ext.extend(Ext.form.Field, {
	initComponent: function () {
		...
		_gridPanel = new ExtMvc.EmployeeGridPanel(_cfg);
		...
		Ext.apply(_this, {
			onRender: function (ct, position) {
				...
				ExtMvc.EmployeeListField.superclass.onRender.call(_this, ct, position);
			...
			onResize: function (w, h, aw, ah) {
				ExtMvc.EmployeeListField.superclass.onResize.apply(_this, arguments);
				_gridPanel.setSize(w, h);
			},
			onEnable: function () {
				ExtMvc.EmployeeListField.superclass.onEnable.apply(_this, arguments);
				_gridPanel.enable();
			},
			onDisable: function () {
				ExtMvc.EmployeeListField.superclass.onDisable.apply(_this, arguments);
				_gridPanel.disable();
			},
			beforeDestroy: function () {
				Ext.destroy(_gridPanel);
				ExtMvc.EmployeeListField.superclass.beforeDestroy.apply(_this, arguments);
			},
			setValue: function (v) {
				_gridPanel.getStore().proxy.data.items = v;
				_gridPanel.getStore().load();
				return ExtMvc.EmployeeListField.superclass.setValue.apply(_this, arguments);
			},
			getValue: function () {
		...
		ExtMvc.EmployeeListField.superclass.initComponent.apply(_this, arguments);
	}
});
