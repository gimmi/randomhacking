...
ExtMvc.Ns.CategoryNormalSearchPanel = Ext.extend(Ext.Panel, {
	initComponent: function () {
		...
		_searchFormPanel = new ExtMvc.Ns.CategoryNormalSearchFormPanel({
		...
		_store = new Ext.data.Store({
			...
			proxy: new Rpc.JsonPostHttpProxy({
				url: 'NsCategory/SearchNormal'
			}),
			remoteSort: true,
			reader: new ExtMvc.Ns.CategoryJsonReader(),
			listeners: {
		...
		_gridPanel = new ExtMvc.Ns.CategoryGridPanel({
			region: 'center',
			store: _store,
			bbar: _pagingToolbar,
			listeners: {
		...
		ExtMvc.Ns.CategoryNormalSearchPanel.superclass.initComponent.apply(_this, arguments);
