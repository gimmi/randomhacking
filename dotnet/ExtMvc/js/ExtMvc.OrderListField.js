...
ExtMvc.OrderListField = Ext.extend(Ext.form.Field, {
	initComponent: function () {
		...
		_gridPanel = new ExtMvc.OrderGridPanel(_cfg);
		...
		Ext.apply(_this, {
			onRender: function (ct, position) {
				...
				ExtMvc.OrderListField.superclass.onRender.call(_this, ct, position);
			...
			onResize: function (w, h, aw, ah) {
				ExtMvc.OrderListField.superclass.onResize.apply(_this, arguments);
				_gridPanel.setSize(w, h);
			},
			onEnable: function () {
				ExtMvc.OrderListField.superclass.onEnable.apply(_this, arguments);
				_gridPanel.enable();
			},
			onDisable: function () {
				ExtMvc.OrderListField.superclass.onDisable.apply(_this, arguments);
				_gridPanel.disable();
			},
			beforeDestroy: function () {
				Ext.destroy(_gridPanel);
				ExtMvc.OrderListField.superclass.beforeDestroy.apply(_this, arguments);
			},
			setValue: function (v) {
				_gridPanel.getStore().proxy.data.items = v;
				_gridPanel.getStore().load();
				return ExtMvc.OrderListField.superclass.setValue.apply(_this, arguments);
			},
			getValue: function () {
		...
		ExtMvc.OrderListField.superclass.initComponent.apply(_this, arguments);
	}
});
